...
def Category_Dict_Generate(pronunciation_Dict, word_Index_Dict):
    category_Dict = {}
    for target_Word in word_Index_Dict.keys():
        ...
        for compare_Word, compare_Word_Index in word_Index_Dict.items():                
            ...
            if target_Pronunciation[1:] == compare_Pronunciation[1:] and target_Pronunciation[0] != compare_Pronunciation[0] and target_Word != compare_Word:
                ...
                unrelated = False
            if unrelated:
                category_Dict[target_Word, "Unrelated"].append(compare_Word_Index)
            #For test
            if DAS_Neighborhood_Checker(target_Pronunciation, compare_Pronunciation):
                category_Dict[target_Word, "DAS_Neighborhood"].append(compare_Word_Index)

    return category_Dict

def DAS_Neighborhood_Checker(pronunciation1, pronunciation2):
    #Same pronunciation
...
def Pattern_Type_Dict_Generate(trained_Pattern_List, excluded_Pattern_List, excluded_Talker):
    pattern_Type_Dict = {}
    for (word, talker) in trained_Pattern_List:
        pattern_Type_Dict[word, talker] = "Trained"
    for (word, talker) in excluded_Pattern_List:
...
def Adjusted_Length_Dict_Generate(pronunciation_Dict):
...
class Result_Analyzer:
    ...
    def Tensor_Generate(self):
        ...
        result_Tensor = tf.placeholder(self.bit_Type, shape=[None, self.metadata_Dict["Semantic_Size"]])  #[Cycle, Semantic_Size]
        target_Tensor = tf.constant(self.metadata_Dict["Target_Array"], dtype=self.bit_Type)  #[Pattern, Semantic_Size]
    ...
    def Loading_Results(self, result_File):
    ...
    def Data_Generate_by_CS(self, target_Index, cycle_Batch_Size = 200):
        cs_Array_List = []
    ...
    def Data_Generate_by_Activation(self, target_Index):
    ...
    def RT_Generate(self, word, talker, data):
        ...
        if not np.isnan(rt_Dict["Onset", "Absolute"]):
            ...
            rt_Dict["Offset", "Absolute"] = rt_Dict["Onset", "Absolute"]
        if not np.isnan(rt_Dict["Onset", "Relative"]):
            ...
            rt_Dict["Offset", "Relative"] = rt_Dict["Onset", "Relative"]
        if not np.isnan(rt_Dict["Onset", "Time_Dependent"]):
            ...
            rt_Dict["Offset", "Time_Dependent"] = rt_Dict["Onset", "Time_Dependent"]

        return rt_Dict

    def Category_Flow_Generate(self, word, talker, data):
        category_Flow_Dict = {}
        
        for category in ["Target", "Cohort", "Rhyme", "Unrelated"]:
            if len(self.metadata_Dict["Category_Dict"][word, category]) > 0:
                category_Flow_Dict[category] = np.mean(data[self.metadata_Dict["Category_Dict"][word, category],:], axis=0)
            else:
                category_Flow_Dict[category] = np.zeros((data.shape[1])) * np.nan
                
        category_Flow_Dict["All"] = np.mean(data, axis=0)
        category_Flow_Dict["Other_Max"] = np.max(np.delete(data, self.metadata_Dict["Word_Index_Dict"][word], 0), axis=0)
    ...
    def Result_Write(self, reaction_Time = True, categorized_Flow = True, top10_Flow = False, raw_Data = False):
        ...
        for target_Index in range(self.result_Start_Index, self.result_Start_Index + self.result_Array.shape[0]):
            ...
            if pattern_Parameters.Semantic_Mode in ["Word2Vec", "SRV"]:
                data = self.Data_Generate_by_CS(target_Index)
            elif pattern_Parameters.Semantic_Mode == "One-hot":
                data = self.Data_Generate_by_Activation(target_Index)

            rt_Dict = self.RT_Generate(word, talker, data)
    ...
    def Extract_RT(self, word, talker, rt_Dict):
        line_List = [            
    ...
    def Extract_Raw_Data(self, word, talker, data):
    ...
    def Extract_Categorized_Flow(self, word, talker, rt_Dict, category_Flow_Dict):
    ...
    def Extract_Top10_Flow(self, word, talker, data, rt_Dict):
